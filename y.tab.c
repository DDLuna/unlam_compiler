...
{
       0,   208,   208,   211,   212,   213,   216,   219,   220,   223,
     224,   225,   226,   230,   231,   232,   236,   237,   240,   241,
     244,   249,   256,   263,   272,   278,   283,   288,   295,   301,
     304,   305,   308,   311,   312,   313,   314,   317,   320,   321,
...
#line 336 "Sintactico.y"
    {(yyval.ast) = crear_nodo("MOD", verificar_conflicto_tipos(obtener_tipo_arbol((yyvsp[(1) - (3)].ast)), obtener_tipo_arbol((yyvsp[(3) - (3)].ast))), (yyvsp[(1) - (3)].ast), (yyvsp[(3) - (3)].ast));}
    break;
...
#line 342 "Sintactico.y"
    {guardar_tabla_de_simbolos((yyvsp[(1) - (1)].num), INT, !ES_ID); (yyval.ast) = crear_hoja(buscar_con_nombre_modificado((yyvsp[(1) - (1)].num)), INT);}
    break;
...
#line 343 "Sintactico.y"
    {guardar_tabla_de_simbolos((yyvsp[(1) - (1)].real),FLOAT, !ES_ID); (yyval.ast) = crear_hoja(buscar_con_nombre_modificado((yyvsp[(1) - (1)].real)), FLOAT);}
    break;
...
void procesar_nodo(arbol* a){
    ...
    if (strcmp(a->nodo, "/") == 0) {
        fprintf(file,"\n\t; DIVIDE \n");
        fprintf(file,"\t%s %s\n", cargar, a->izq->nodo);
        fprintf(file,"\t%s %s\n", instruccion_dividir, a->der->nodo);
        //fprintf(file,"\tFDIV\n");
        a->nodo = "@DIVIDIR";
        fprintf(file,"\t%s %s\n", asignar, a->nodo);
        limpiar_pila();
